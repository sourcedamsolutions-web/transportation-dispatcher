...
function buildDefaultBoard(routeRows) {
  ...
  for (const r of routeRows) {
    ...
    const prefill = (String(r.code).endsWith('A') ? r.default_assistant : r.default_driver) || 'OPEN';
    cells[0] = prefill;
...
function buildPeopleList(routeRows) {
  const set = new Set();
  for (const r of routeRows) {
    if (r.default_driver && r.default_driver !== 'OPEN') set.add(r.default_driver);
    if (r.default_assistant && r.default_assistant !== 'OPEN') set.add(r.default_assistant);
  }
  // Known relief + supervisors (can be refined later)
  ['Julia','Noel','Lizette','Myra','Jeff','Ray','Nic','Rachael'].forEach(n => set.add(n));
...
// Health
app.get('/api/health', (req, res) => res.json({ ok: true, app: APP_NAME, version: APP_VERSION }));
...
// Day Sheet Board (inventory-like)
...
app.get('/api/people', authRequired, async (req, res) => {
  const routeRows = await getRouteCodesOrdered();
  res.json({ people: buildPeopleList(routeRows) });
});

app.get('/api/callouts', authRequired, async (req, res) => {
  const date = req.query.date;
  if (!date) return res.status(400).send('date required');
  ...
  if (r.rowCount === 0) {
    return res.json({
      date,
      data: { drivers: { am: [], pm: [] }, assistants: { am: [], pm: [] } }
    });
  }
  res.json({ date, data: r.rows[0].data || {} });
});

app.post('/api/callouts', authRequired, async (req, res) => {
  const { date, data } = req.body || {};
  if (!date) return res.status(400).send('date required');
  await pool.query(
    `insert into callouts (day, data) values ($1, $2::jsonb)
     on conflict (day) do update set data=$2::jsonb, updated_at=now()`,
